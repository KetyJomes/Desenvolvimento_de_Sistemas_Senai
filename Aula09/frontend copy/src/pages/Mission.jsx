...
function Mission() {
  ...
  const [produts, setMission] = useState([])
  ...
  const handleCreatTask = () => {
    navigate('/Mission Creat')
  }

  const deleteMisson = async (id) => {
    Swal.fire({
...
<div style={{ height: '100vh', width: '100vw', backgroundColor: '#A3B18C', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '20px' }}>
...

  {
    tasks.map((mission) => {
      return (
        <div key={mission._id} style={{ backgroundColor: 'white', padding: '15px', borderRadius: '10px', marginBottom: '15px', width: '50%', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', marginBottom: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-start' }}>
              <button onClick={() => deleteMisson(mission._id)} style={buttonStyleDelete}>Deletar</button>
              <button onClick={() => updateMission(mission._id)} style={buttonStyleUpdate}>Alterar</button>
            </div>
          </div>
        </div>
      )
  ...
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
    <button onClick={handleCreatTask} style={buttonStyle}>
      Mission Creat
    </button>
  </div>
</div>
